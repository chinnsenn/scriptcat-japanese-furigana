...
| 能力 | 行为 |
| --- | --- |
| 精准采集 | 支持选中文本、正文区域、整页；按 `lang`、页面语言、假名与上下文识别 Japanese/Other/Ambiguous 区间，并排除页面外壳 |
...
| 视频防暂停 | 当前站点显式启用后，页面失焦或隐藏时自动恢复由站点脚本触发的非用户暂停 |
...
- `设置 Yahoo Client ID`：使用宽型四行输入框完整展示长 Client ID，支持自动折行、字符计数、保存、清空和取消。
...
- `切换当前站点视频防暂停`：管理按 origin 隔离的视频失焦防暂停开关。
- `清理当前站点读音缓存`：清理当前引擎内存条目与当前 origin 的持久缓存。
- `查看本次实际发送范围`：显示真实进入 Yahoo 请求的次数、字符数和受限摘要。

## 采集、标注与撤销

脚本按元素 `lang`、页面语言、假名和相邻上下文，把含汉字正文切成 Japanese、Other 与 Ambiguous 区间。Japanese 区间进入 Yahoo 分析；Other 与 Ambiguous 分别跳过并显示在诊断面板；用户选中的范围作为显式意图强制分析。每个区间持续携带 DOM 来源、分类证据和块内偏移，API 结果由这些稳定信息映射回原始文本节点。

选中文本只处理 Range 覆盖的字符；正文区域优先使用 `main`、`article` 或 `[role="main"]`，缺少语义容器时回退到 `body`；整页范围从 `body` 采集。导航、页眉页脚、侧栏、表单、代码、可编辑区域、隐藏内容与已有 ruby 在语言分类前排除。
...
视频防暂停默认关闭，并按 `location.origin` 保存。启用后，脚本在页面早期伪装可见性，捕获 `visibilitychange`、`blur`、`pagehide` 等失焦信号，追踪页面上的现有和动态 `<video>`。如果视频曾处于播放状态，随后在失焦窗口内被页面脚本暂停，脚本会调用 `video.play()` 恢复。

脚本会记录最近的鼠标、触摸和键盘操作。用户刚操作过的暂停视为真实暂停意图，保持暂停状态。
...
诊断面板显示处理进度、标注字数、Other/Ambiguous 跳过数、API 调用、缓存命中、限流等待、瞬时重试、失败片段与最近耗时。

## 隐私模型

Other 与 Ambiguous 区间停留在当前页面。Japanese 区间或用户强制选择范围在缓存缺失时分段发送到 `https://jlp.yahooapis.jp/jsonrpc`，同时携带用户配置的 Client ID。
...
视频防暂停白名单同样按 `location.origin` 保存。该功能只读取页面视频状态，不发送视频地址、标题或播放记录。

## 已知边界

- Yahoo 单次请求上限为 4KB，脚本使用约 3KB 的保守分块。
- 跨节点词元需要形成连续安全区间；混入被排除内容的区间会跳过。
- 未声明语言的纯汉字短语归入 Ambiguous；使用页面 `lang="ja"` 或选中文本可以提供明确证据。
- 人名、地名、作品名与自定义读音仍可能产生歧义。
- 网站框架重渲染后，脚本通过脏根队列增量恢复注音。
- 浏览器限制 `localStorage` 时，脚本继续使用内存缓存与远程分析。
- 浏览器后台节流、移动端自动播放策略、系统省电策略和跨域 iframe 权限会影响视频防暂停恢复能力。
...
源码按 `reading/page/media/scriptcat/main` 领域 Module 维护，esbuild 生成无运行时依赖的单文件安装产物。`npm run check` 会重新构建产物、执行 JavaScript 语法检查和全部 Node 回归测试。浏览器端场景位于 `work/smoke.html`。
