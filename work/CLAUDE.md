...
成员清单
japanese-furigana.test.cjs: 验证共享算法、缓存、可取消 Yahoo Adapter、Retry-After、分块降级、读音对齐和圆形图标按钮状态
...
smoke.html: 加载 outputs 构建产物并模拟 ScriptCat/Yahoo，手动预览 Client ID 配置框，并通过 `?auto=1` 自动验证首次标注、动态正文与完整撤销
