...
media-guard.test.cjs: 通过 jsdom 验证站点级视频防暂停的失焦恢复、用户暂停保留、动态 video 追踪和可见性恢复
dom.test.cjs: 通过 jsdom 验证语言证据与来源偏移、三种范围、跨内联节点标注、元素身份与完整撤销
scriptcat.test.cjs: 通过 jsdom 与 ScriptCat Adapter Interface 验证宽型 Client ID 配置框、站点许可/白名单、视频防暂停开关、发送审计、范围与缓存菜单
